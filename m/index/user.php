...
// 登陆欢迎页面
if ($ac == 'index') {
	$sellcars=$db->row_count('cars'," issell=0 and uid=".$_SESSION['USER_ID']);
	$tpl -> assign('sellcars', $sellcars);

	$userinfo['last_login_time'] = date("Y-m-d H:i:s", $userinfo['last_login_time']);
	$tpl -> assign('user', $userinfo); 
	$tpl -> display('m/user.html');
	exit;
} 
// 退出登录
...
elseif ($ac == 'addcar' || $ac == 'editcar') {
	...
	// 转向添加或修改页面
		...
		$tpl -> display('m/user_addcar.html');
		exit;
	} 
}
// 添加图片
