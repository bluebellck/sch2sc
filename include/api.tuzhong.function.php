...
function api_tuzhong_all($data,$uid,$type){
	...
	$arr['aid'] = 11;
	$arr['cid'] = 88;
	$arr['p_ontime'] = strtotime($data['p_year'].'-'.$data['p_month'].'-01');
...
function api_tuzhong_function($data,$uid,$type){
	$arr = array();
	$arr['type'] = $type;
	$arr['uid'] = $uid;
	$arr['cheyuan_id'] = $data['cheyuan_id'];
