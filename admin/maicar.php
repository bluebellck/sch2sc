...
if ($ac == 'list') {	
	...
	$Page = new Page($db -> tb_prefix . 'maicar', $where, '*', '50', 'p_id desc');
	$list = $Page -> get_data();
	$page = $Page -> page;
	...
	$tpl -> display('admin/maicar_list.html');
	exit;
} 
elseif ($ac == 'del') {
	$p_id = isset($_GET['id']) ? intval($_GET['id']) : showmsg('缺少ID', -1);
	$car = $db -> row_select_one('maicar', "p_id=$p_id");
	if(!empty($car['p_pics'])) {
	...
	$rs = $db -> row_delete('maicar', "p_id=$p_id");
} 
// 批量删除
elseif ($ac == 'bulkdel') {
	if (empty($_POST['bulkid'])) showmsg('没有选中任何项', -1);
	foreach($_POST['bulkid'] as $key => $value) {
		$car = $db -> row_select_one('maicar', "p_id=" . $value);
		if(!empty($car['p_pics'])) {
			$pic_list = explode('|', $car['p_pics']);
			foreach($pic_list as $key => $value) {
				$pic = str_replace(WEB_PATH . '/', '', $value);
	...
	$rs = $db -> row_delete('maicar', "p_id in($str_id)");
}
//添加
elseif ($ac == 'assess')
{
    ...
    if (submitcheck('a'))
    {
		...
		$rs = $db->row_update('maicar',$post,"p_id=".intval($_POST['id']));
    }
    //转向添加或修改页面
    else
    {
        if (empty($_GET['id'])) {
			...
			$data = $db->row_select_one('maicar',"p_id=".intval($_GET['id']));
		}
